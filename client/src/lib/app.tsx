...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    fetch("/api/local-state")
      ...
      .then((remote: Partial<Persisted> | null) => {
        if (cancelled) return
        const hasRemoteData = Boolean(remote?.students?.length || remote?.teachers?.length || remote?.classrooms?.length)
        if (hasRemoteData) {
          setStore({ students: remote?.students ?? [], teachers: remote?.teachers ?? [], classrooms: remote?.classrooms ?? [] })
        }
        setHydrated(true)
      })
